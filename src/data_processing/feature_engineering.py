...
class FeatureEngineer:
    ...
    def create_temporal_features(
        self, df: pd.DataFrame, date_col: str = "visit_date"
    ) -> pd.DataFrame:
        """
        ...
        df_feat["days_since_epoch"] = (
            df_feat[date_col] - pd.Timestamp("2020-01-01")
        ).dt.days
    ...
    def create_lagged_features(
        self, df: pd.DataFrame, value_cols: List[str], lags: List[int] = [1, 3, 7, 14]
    ) -> pd.DataFrame:
        """
    ...
    def create_rolling_features(
        self, df: pd.DataFrame, value_cols: List[str], windows: List[int] = [3, 7, 14]
    ) -> pd.DataFrame:
        """
        ...
        for col in value_cols:
            if col in df_feat.columns:
                for window in windows:
                    df_feat[f"{col}_rolling_mean_{window}d"] = (
                        df_feat[col].rolling(window).mean()
                    )
                    df_feat[f"{col}_rolling_std_{window}d"] = (
                        df_feat[col].rolling(window).std()
                    )
                    df_feat[f"{col}_rolling_max_{window}d"] = (
                        df_feat[col].rolling(window).max()
                    )
                    df_feat[f"{col}_rolling_min_{window}d"] = (
                        df_feat[col].rolling(window).min()
                    )
    ...
    def create_interaction_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if "is_weekend" in df_feat.columns and "arrival_hour" in df_feat.columns:
            df_feat["weekend_evening"] = (
                (df_feat["is_weekend"] == 1) & (df_feat["arrival_hour"].between(18, 23))
            ).astype(int)

        if "age_group" in df_feat.columns and "has_insurance" in df_feat.columns:
            df_feat["senior_uninsured"] = (
                (df_feat["age_group"] == "65+") & (df_feat["has_insurance"] == 0)
            ).astype(int)
        ...
        if "month" in df_feat.columns:
            df_feat["is_flu_season"] = (
                df_feat["month"].isin([10, 11, 12, 1, 2, 3]).astype(int)
            )
    ...
    def create_aggregated_features(
        self, df: pd.DataFrame, group_cols: List[str], agg_col: str
    ) -> pd.DataFrame:
        """
        ...
        for group_col in group_cols:
            if group_col in df_feat.columns and agg_col in df_feat.columns:
                group_stats = (
                    df_feat.groupby(group_col)[agg_col]
    ...
    def create_all_features(
        ...
        self.logger.info(
            f"Feature engineering complete: {final_cols - initial_cols} new features created"
        )
