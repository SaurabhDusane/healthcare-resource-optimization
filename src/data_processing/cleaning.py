...
class DataCleaner:
    ...
    def clean_nhamcs_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if "ARRTIME" in df_clean.columns:
            df_clean["arrival_hour"] = (
                pd.to_numeric(df_clean["ARRTIME"], errors="coerce") // 100
            )
        ...
        self.logger.info(
            f"Cleaning complete: {len(df_clean)} records, {len(df_clean.columns)} columns"
        )
    ...
    def clean_scraped_data(self, df: pd.DataFrame, source: str) -> pd.DataFrame:
        ...
        duplicate_cols = (
            ["date", "title"] if "title" in df_clean.columns else ["date", "text"]
        )
    ...
    def merge_datasets(
        ...
        twitter_df: Optional[pd.DataFrame] = None,
    ) -> pd.DataFrame:
        """
        ...
        # Single named join key (normalized to midnight) reused by every
        # source merge, so we never leak anonymous ``key_0`` columns.
        has_visit_date = "visit_date" in merged_df.columns
        if has_visit_date:
            merged_df["_join_date"] = merged_df["visit_date"].dt.normalize()

        if cdc_df is not None and not cdc_df.empty:
            cdc_agg = (
                cdc_df.groupby(cdc_df["date"].dt.date)
                .agg({"title": "count", "keywords": lambda x: ", ".join(x)})
            ...
            if has_visit_date:
                merged_df = merged_df.merge(
                    cdc_agg, left_on="_join_date", right_index=True, how="left"
                )

        if reddit_df is not None and not reddit_df.empty:
            reddit_agg = (
                reddit_df.groupby(reddit_df["date"].dt.date)
                .agg(
                    {
                        "post_id": "count",
                ...
                .rename(
                    columns={
                        "post_id": "reddit_posts",
                        "sentiment_polarity": "reddit_sentiment",
                    }
                )
            )
            reddit_agg.index = pd.to_datetime(reddit_agg.index)

            reddit_agg["reddit_sentiment_7d"] = (
                reddit_agg["reddit_sentiment"].rolling(7).mean()
            )

            if has_visit_date:
                merged_df = merged_df.merge(
                    reddit_agg, left_on="_join_date", right_index=True, how="left"
                )

        if twitter_df is not None and not twitter_df.empty:
            twitter_agg = (
                twitter_df.groupby(twitter_df["date"].dt.date)
                .agg(
                    {
                        "tweet_id": "count",
                        "sentiment_polarity": "mean",
            ...
            if has_visit_date:
                merged_df = merged_df.merge(
                    twitter_agg, left_on="_join_date", right_index=True, how="left"
                )

        scraped_cols = [
            col
            for col in merged_df.columns
            if any(x in col for x in ["news", "reddit", "twitter", "sentiment"])
        ]
        merged_df[scraped_cols] = merged_df[scraped_cols].fillna(0)

        if "_join_date" in merged_df.columns:
            merged_df = merged_df.drop(columns="_join_date")

        self.logger.info(
            f"Merge complete: {len(merged_df)} records, {len(merged_df.columns)} columns"
        )
