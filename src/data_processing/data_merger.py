...
class DataMerger:
    ...
    def merge_all_sources(
        self, nhamcs_df, cdc_df=None, reddit_df=None, twitter_df=None
    ):
        ...
        if twitter_df is not None and not twitter_df.empty:
            twitter_agg = (
                ...
                .size()
                .to_frame("twitter_count")
            )
            twitter_agg.index = pd.to_datetime(twitter_agg.index)
            merged_df = merged_df.merge(
                twitter_agg, left_on="visit_date", right_index=True, how="left"
            )
